...
- giải thích indexes, execution plans và sql operations bằng mental model và boundary;
...
Access path, statistics, actual plan, Query Store, backups và capacity. Đây là boundary nơi một quyết định nhỏ có thể đổi correctness, latency, security và operational ownership.

## Tổng quan / Overview

~~~mermaid
flowchart LR
    A["Input / workload"] --> B["Indexes, Execution Plans và SQL Operations"] --> C["State / result"] --> D["Evidence / decision"]
~~~
...
Access path, statistics, actual plan, Query Store, backups và capacity. Học stable concept trước version/tool syntax; mọi claim production phải có measurement hoặc source.
...
Bắt đầu từ requirement, chọn primitive, đặt safety bound, quan sát behavior và xác định owner. Access path, statistics, actual plan, Query Store, backups và capacity. Không coi framework/platform abstraction là proof của correctness.

## Minimal Example

~~~sql
CREATE INDEX IX_Items_Tenant_Id ON dbo.Items(TenantId, Id);
~~~
...
Access path, statistics, actual plan, Query Store, backups và capacity. Production path bổ sung contract test, structured telemetry, failure classification, rollout/rollback và data/privacy policy.
...
Indexes, Execution Plans và SQL Operations trở thành architectural boundary khi ảnh hưởng ownership, consistency, deployment, capacity hoặc team topology. Chọn phương án đơn giản nhất thỏa NFR; document điều gì đổi ở 10x/100x và trigger migrate.
...
Tạo một experiment bounded cho indexes, execution plans và sql operations: ghi workload, expected output, failure scenario và safety bound; chạy baseline rồi so sánh; lưu decision note. Không đưa credential, production data hoặc diagnostic artifact nhạy cảm vào repository.

## Exit Criteria

- Giải thích được access path, statistics, actual plan, query store, backups và capacity..
