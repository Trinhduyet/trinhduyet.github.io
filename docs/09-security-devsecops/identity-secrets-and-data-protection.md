# Identity, Secrets và Data Protection

> [← Module 09 overview](README.md) · [Security và DevSecOps references](references.md)

## Mục tiêu / Learning Objectives

- giải thích identity, secrets và data protection bằng mental model và boundary;
...
Authentication, authorization, key lifecycle, secret rotation, encryption và least privilege. Đây là boundary nơi một quyết định nhỏ có thể đổi correctness, latency, security và operational ownership.
...
Authentication, authorization, key lifecycle, secret rotation, encryption và least privilege. Học stable concept trước version/tool syntax; mọi claim production phải có measurement hoặc source.
...
Bắt đầu từ requirement, chọn primitive, đặt safety bound, quan sát behavior và xác định owner. Authentication, authorization, key lifecycle, secret rotation, encryption và least privilege. Không coi framework/platform abstraction là proof của correctness.

## Minimal Example

~~~text
identity -> policy -> resource -> audit
~~~
...
Authentication, authorization, key lifecycle, secret rotation, encryption và least privilege. Production path bổ sung contract test, structured telemetry, failure classification, rollout/rollback và data/privacy policy.
...
Identity, Secrets và Data Protection trở thành architectural boundary khi ảnh hưởng ownership, consistency, deployment, capacity hoặc team topology. Chọn phương án đơn giản nhất thỏa NFR; document điều gì đổi ở 10x/100x và trigger migrate.
...
Tạo một experiment bounded cho identity, secrets và data protection: ghi workload, expected output, failure scenario và safety bound; chạy baseline rồi so sánh; lưu decision note. Không đưa credential, production data hoặc diagnostic artifact nhạy cảm vào repository.

## Exit Criteria

- Giải thích được authentication, authorization, key lifecycle, secret rotation, encryption và least privilege..
