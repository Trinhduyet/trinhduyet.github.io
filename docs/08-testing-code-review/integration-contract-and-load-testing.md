# Integration, Contract và Load Testing

> [← Module 08 overview](README.md) · [Testing và Code Review references](references.md)

## Mục tiêu / Learning Objectives

- giải thích integration, contract và load testing bằng mental model và boundary;
...
Real dependencies, schema/HTTP contract, load model và failure injection. Đây là boundary nơi một quyết định nhỏ có thể đổi correctness, latency, security và operational ownership.
...
Real dependencies, schema/HTTP contract, load model và failure injection. Học stable concept trước version/tool syntax; mọi claim production phải có measurement hoặc source.
...
Bắt đầu từ requirement, chọn primitive, đặt safety bound, quan sát behavior và xác định owner. Real dependencies, schema/HTTP contract, load model và failure injection. Không coi framework/platform abstraction là proof của correctness.
...
Real dependencies, schema/HTTP contract, load model và failure injection. Production path bổ sung contract test, structured telemetry, failure classification, rollout/rollback và data/privacy policy.
...
Integration, Contract và Load Testing trở thành architectural boundary khi ảnh hưởng ownership, consistency, deployment, capacity hoặc team topology. Chọn phương án đơn giản nhất thỏa NFR; document điều gì đổi ở 10x/100x và trigger migrate.
...
Tạo một experiment bounded cho integration, contract và load testing: ghi workload, expected output, failure scenario và safety bound; chạy baseline rồi so sánh; lưu decision note. Không đưa credential, production data hoặc diagnostic artifact nhạy cảm vào repository.

## Exit Criteria

- Giải thích được real dependencies, schema/http contract, load model và failure injection..
