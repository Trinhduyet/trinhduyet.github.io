# Terraform State, Modules và Drift

> [← Module 13 overview](README.md) · [DevOps và IaC references](references.md)

## Mục tiêu / Learning Objectives

- giải thích terraform state, modules và drift bằng mental model và boundary;
...
Plan/apply, remote state locking, module composition, policy và drift detection. Đây là boundary nơi một quyết định nhỏ có thể đổi correctness, latency, security và operational ownership.
...
Plan/apply, remote state locking, module composition, policy và drift detection. Học stable concept trước version/tool syntax; mọi claim production phải có measurement hoặc source.
...
Bắt đầu từ requirement, chọn primitive, đặt safety bound, quan sát behavior và xác định owner. Plan/apply, remote state locking, module composition, policy và drift detection. Không coi framework/platform abstraction là proof của correctness.

## Minimal Example

~~~yaml
terraform init -> validate -> plan -> review -> apply
~~~
...
Plan/apply, remote state locking, module composition, policy và drift detection. Production path bổ sung contract test, structured telemetry, failure classification, rollout/rollback và data/privacy policy.
...
Terraform State, Modules và Drift trở thành architectural boundary khi ảnh hưởng ownership, consistency, deployment, capacity hoặc team topology. Chọn phương án đơn giản nhất thỏa NFR; document điều gì đổi ở 10x/100x và trigger migrate.
...
Tạo một experiment bounded cho terraform state, modules và drift: ghi workload, expected output, failure scenario và safety bound; chạy baseline rồi so sánh; lưu decision note. Không đưa credential, production data hoặc diagnostic artifact nhạy cảm vào repository.

## Exit Criteria

- Giải thích được plan/apply, remote state locking, module composition, policy và drift detection..
