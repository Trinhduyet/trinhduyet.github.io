# CI/CD, Artifacts và Promotion

> [← Module 13 overview](README.md) · [DevOps và IaC references](references.md)

## Mục tiêu / Learning Objectives

- giải thích ci/cd, artifacts và promotion bằng mental model và boundary;
...
Build/test/scan, immutable artifact, environment gates, provenance và rollback. Đây là boundary nơi một quyết định nhỏ có thể đổi correctness, latency, security và operational ownership.
...
Build/test/scan, immutable artifact, environment gates, provenance và rollback. Học stable concept trước version/tool syntax; mọi claim production phải có measurement hoặc source.
...
Bắt đầu từ requirement, chọn primitive, đặt safety bound, quan sát behavior và xác định owner. Build/test/scan, immutable artifact, environment gates, provenance và rollback. Không coi framework/platform abstraction là proof của correctness.

## Minimal Example

~~~yaml
restore -> test -> scan -> artifact -> promote
~~~
...
Build/test/scan, immutable artifact, environment gates, provenance và rollback. Production path bổ sung contract test, structured telemetry, failure classification, rollout/rollback và data/privacy policy.
...
CI/CD, Artifacts và Promotion trở thành architectural boundary khi ảnh hưởng ownership, consistency, deployment, capacity hoặc team topology. Chọn phương án đơn giản nhất thỏa NFR; document điều gì đổi ở 10x/100x và trigger migrate.
...
Tạo một experiment bounded cho ci/cd, artifacts và promotion: ghi workload, expected output, failure scenario và safety bound; chạy baseline rồi so sánh; lưu decision note. Không đưa credential, production data hoặc diagnostic artifact nhạy cảm vào repository.

## Exit Criteria

- Giải thích được build/test/scan, immutable artifact, environment gates, provenance và rollback..
